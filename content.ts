...
window.addEventListener("load", async () => {
  const highlightEnabled = await storage.get<boolean>("highlight-enabled")
  const hideBlacklistEnabled = await storage.get<boolean>("hide-blacklist-enabled")
    ...
    ddElements.forEach((dd) => {
      ...
      if (titleElement && titleWhiteList) {
        ...
        titleKeywords.forEach((keyword) => {
          ...
          titleElement.innerHTML = titleElement.innerHTML.replace(regex, `<span style="background-color: yellow;">$1</span>`);
        });
      }
      ...
      if (authorElement && authorWhiteList) {
        ...
        authorKeywords.forEach((keyword) => {
          const regex = new RegExp(`(${keyword})`, "gi");
          authorElement.innerHTML = authorElement.innerHTML.replace(regex, `<span style="background-color: yellow;">$1</span>`);
        });
      }
      ...
      if (commentElement && commentWhiteList) {
        ...
        commentKeywords.forEach((keyword) => {
          const regex = new RegExp(`(${keyword})`, "gi");
          commentElement.innerHTML = commentElement.innerHTML.replace(regex, `<span style="background-color: yellow;">$1</span>`);
        });
      }
    });
  }
